...
const MenuItemCard = React.forwardRef<{ triggerClick: () => void }, MenuItemCardProps>(({ item, isTwoColumns: isTwoColumnsProp, priority = false }, ref) => {
  ...
  return (
    <>
      <div className={`food-item rounded-lg overflow-hidden shadow-md p-3 sm:p-4 flex flex-col ${isUnavailable(item) ? 'opacity-50 grayscale' : ''}`} style={{ backgroundColor: settings.cor_background_header, color: settings.cor_fonte }} data-product-id={item.id}>
        <div
