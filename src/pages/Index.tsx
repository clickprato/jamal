...
const Index = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const profile = await getProfile(currentUser.id).catch(() => null);
        const phone = profile?.phone || (currentUser as any).phoneNumber || null;
        const variants = phone ? phoneVariants(phone) : [];
        const phones = variants.length ? variants : phone ? [phone] : [];
        if (!phones.length) return;
        const recompensas = await getRecompensasCliente(phones);
